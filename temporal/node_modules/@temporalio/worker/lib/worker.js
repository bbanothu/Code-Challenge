...
class Worker {
    ...
    static async create(options) {
        const logger = (0, logger_1.withMetadata)(runtime_1.Runtime.instance().logger, {
            sdkComponent: common_1.SdkComponent.worker,
            taskQueue: options.taskQueue ?? 'default',
        });
        const nativeWorkerCtor = this.nativeWorkerCtor;
        const compiledOptions = (0, worker_options_1.compileWorkerOptions)(options, logger);
    ...
    static async constructReplayWorker(options) {
        ...
        const compiledOptions = (0, worker_options_1.compileWorkerOptions)(fixedUpOptions, logger);
        const bundle = await this.getOrCreateBundle(compiledOptions, logger);
        if (!bundle) {
    ...
    logger, connection, isReplayWorker = false) {
        this.nativeWorker = nativeWorker;
    ...
    activityOperator() {
        return (0, rxjs_1.pipe)((0, rxutils_1.closeableGroupBy)(({ base64TaskToken }) => base64TaskToken), (0, operators_1.mergeMap)((group$) => {
                ...
                switch (variant) {
                    case 'start': {
                        ...
                        try {
                            ...
                            }), this.logger, this.options.interceptors.activity);
            ...
            }), (0, operators_1.filter)((result) => result !== undefined), (0, operators_1.map)((rest) => proto_1.coresdk.ActivityTaskCompletion.encodeDelimited(rest).finish()));
        }));
    }
    /**
    ...
    async createWorkflow(activation, initWorkflowJob) {
        ...
        const { workflowId, randomnessSeed, workflowType, parentWorkflowInfo, workflowExecutionTimeout, workflowRunTimeout, workflowTaskTimeout, continuedFromExecutionRunId, firstExecutionRunId, retryPolicy, attempt, cronSchedule, workflowExecutionExpirationTime, cronScheduleToScheduleInterval, } = initWorkflowJob;
